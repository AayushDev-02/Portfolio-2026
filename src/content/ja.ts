...
const about = {
  ...
  portraitAlt:
    "モノクロの抽象的なイメージ。柔らかくぼかした淡い形が濃い影と交差する構成。写真が用意されるまでの仮画像です。",
  rows: [
    {
      question: "担当している領域",
      items: [
...
const skills = {
  ...
  diagram: {
    title: "技術スタックの5つの層",
    desc: "ブラウザ側から順に5層。フロントエンド：TypeScript、JavaScript、React、Next.js、Vue.js、Vite、Tailwind CSS、MapLibre、Apache ECharts。API・サービス：Python、Java、C++、FastAPI、Node.js、Express.js、Django、Flask、Spring Boot、REST API、非同期・マルチスレッド処理。AI・検索（強調表示）：RAG、大規模言語モデル、LangChain、OpenAI API、Hugging Face Transformers、PyTorch、プロンプト設計、チャンク設計・リランキング、セマンティック検索・ベクトル検索、embeddings、pgvector、Pinecone、FAISS、検索基盤の最適化、検索精度の評価、OCRパイプライン、ローカルLLMの運用。データ：SQL、PostgreSQL、MySQL、MongoDB、Redis、PostGIS、QGIS、pandas、NumPy、Matplotlib、Jupyter Notebook、PyMuPDF。クラウド・デリバリー：AWS、Microsoft Azure、Docker、GitHub Actions、CI/CD、DigitalOcean、Vercel、Cloudflare、Bash。",
  } satisfies StackDiagram,
  certificationsLabel: "保有資格",
  certifications: [
