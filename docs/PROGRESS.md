...
Briefs: `docs/STAGE12-MOTION.md` + `docs/REFERENCE-B-TEARDOWN.md`.
**The no-library recommendation is withdrawn** — reference B raised the motion
target to Stage 15's level, and building vanilla now then rewriting in GSAP is waste.

- [x] `pnpm add gsap`; register ScrollTrigger + SplitText only
- [x] Log the reversal in DECISIONS.md with measured before/after first-load JS (~40KB gz expected)
- [x] **A** — text scramble on headings (Latin only; `ja` fades instead)
- [x] **B** — count-up on the four RESULTS figures (`tabular-nums`, width reserved)
- [x] **C** — hairline draw-in on section rules (`scaleX`, origin left, ~40ms stagger)
- [x] **D** — cursor crosshair (`pointer: fine`, rAF + `translate3d`, `aria-hidden`)
- [x] **E** — scroll-linked section counter (one observer, `rootMargin: -50%`)
...
- [x] `ja` checked at 360/768 — no per-character effect on CJK text; headings take the fade/plain path
...
- [ ] Lighthouse ≥ 95 both locales; LCP still < 1.5s on a real deploy (local CI runner remains borderline)
**DoD:** all five live; first-load total < 120KB and app code < 15KB; budgets pass; page complete with motion disabled — [ ]

---

## Stage 14 — Hero shader portrait
Briefs: `docs/REFERENCE-B-TEARDOWN.md`, `docs/STAGE14-3D-MOTION.md`.
**Reference B's hero contains no Three.js** — it is one photo through a WebGL
fragment shader. The 150KB budget conflict does not apply.

- [ ] Take the photograph (dark background, side light, face off-centre)
- [ ] Pick the route: raw WebGL / OGL (recommended) / Unicorn.studio
- [ ] Or keep the point-cloud canvas preview instead — decide with the real photo in both
- [ ] Poster stays the LCP element; canvas mounts after `load`
...
**DoD:** first-load total < 120KB (app code < 15KB) and LCP < 1.5s with canvas off; deferred 3D under its own ceiling; site complete with every kill switch tripped — [ ]
...
|---|---|---|
| 2026-08-31 | 12 | Resolved the +49KB vendor regression in order: the analyzer showed static GSAP chunks of 19.8KB + 24.8KB + 6.9KB gzip in the initial route; after the fix those 19.8KB + 17.4KB + 8.2KB + 4.0KB + 3.3KB chunks are async and absent from the initial `/[locale]` manifest. No section became a client component. Added the dev-only analyzer, kept GSAP core/plugins behind first scroll (first fine-pointer move for the cursor), and fixed cursor cleanup so an unactivated leaf does not load GSAP on unmount. The budget contradiction is now explicit: app code remains gated at 15KB, browser-visible total at 120KB because the measured framework floor is 100.8KB; the current build is 13.8KB app / 114.3KB total. The local Lighthouse runner is still intermittently over its 2.5s assertion, so CI's remaining failure needs the exact assertion from the run logs before changing anything. |
| 2026-08-31 | 12 | Installed Playwright 1.62.1 + Chromium and verified local `/en` and `/ja` at 360px: both pages rendered meaningful content, had no browser console errors, and no horizontal overflow. GitHub Actions run `33349709050` was checked via the public API: checkout, install, typecheck/lint, build, and bundle budget passed; only the Lighthouse step failed. GitHub log download is permission-gated for this repository, so the exact Lighthouse assertion is not exposed anonymously. |
| 2026-08-31 | 12 | Reversed the withdrawn no-library recommendation per REFERENCE-B: installed GSAP 3.15, registering only ScrollTrigger and SplitText. Migrated heading scramble (Latin-only with explicit JA fade), RESULTS count-up (reserved-width/tabular numerals), hairline draw-in, fine-pointer crosshair using `quickTo`, and the scroll-linked section counter; all GSAP triggers/tweens are killed on unmount and reduced-motion paths render finished state. Five implementation commits plus dependency/cleanup commits. Production build passes; first-load JS is 159kB for both `/en` and `/ja` versus the documented 110kB baseline (+49kB, above the ~40kB expectation and needs bundle investigation). Browser verification was blocked because the listed agent-browser command and Playwright are not installed in this environment; Lighthouse and real 360/768 JA checks remain open. Stage 12 intentionally remains open for human motion/reduced-motion review. |
