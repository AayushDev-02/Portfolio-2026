...
- [x] **No library.** Not an estimate: `motion` was installed and built earlier in this project — `LazyMotion` + `domAnimation` + `strict` costs **38.3KB gz** (app code 11.4 → 49.7KB against a 15KB budget). `useInView` alone fits at 0.6KB and was still rejected, since the next `m.div` adds the other 38KB. Logged in DECISIONS.md
- [x] **A** — text scramble on headings. **Guarded by character, not locale**: the English copy names Japanese organisations, so an EN heading can legitimately contain CJK and a locale branch would have scrambled it. Printable ASCII only
- [x] **B** — count-up on the four RESULTS figures. The figures are not plain numbers (`1,400`, `<1`, `40%`), so the parser counts only the numeric core and holds prefix/suffix fixed
- [x] **C** — hairline draw-in, `scaleX(0)→scaleX(1)` from the left, 40ms stagger. A real element, not a border — `transform` cannot touch a border and `width` would force layout every frame
- [x] **D** — cursor crosshair. Three gates: `pointer: fine`, not reduced-motion, and the pointer has actually moved. Both media queries are *watched*, not sampled once
- [x] **E** — scroll-linked section counter. One observer, `rootMargin: -50%` top and bottom so it switches at the midpoint. The six static per-section counters are gone, and the dead `Counter` primitive with them
- [-] **F** — boot sequence — **not built**, per the brief and Aayush's instruction
- [x] Every effect renders its finished state under `prefers-reduced-motion` — verified per effect, not assumed
- [x] `ja` checked at 360/768 — **zero CJK headings carry a scramble wrapper** at any width, and no horizontal overflow
- [x] First-load JS reported before/after — **110KB → 111KB**, app code 11.7 → 13.3KB (+1.6KB across five effects)
- [x] Lighthouse ≥ 95 both locales; LCP < 1.5s — Performance 97 both, LCP 1.16s (en) / 1.33s (ja) on production
**DoD:** all five live; first-load JS essentially unchanged; budgets pass; page complete with motion disabled — [ ] *(automated checks all pass; held open for Aayush to view it with motion on and with reduced-motion forced)*

Measured, before → after:

| | before | after |
|---|---|---|
| first-load JS (both locales) | 110 KB | **111 KB** |
| app code (gzipped) | 11.7 KB | **13.3 KB** |
| LCP, production (en / ja) | 1.23s | **1.16s / 1.33s** |
| CLS | 0.0018 | **0.0017** |
| Lighthouse Performance | 97 | **97** |
| axe violations | 0 / 30 rules | **0 / 30 rules** |

**A regression scare that was not one.** Lighthouse against a local `next start`
reports LCP 2.5s and CLS 0.022 with the effects in. Checking out the pre-stage
commit and running the identical measurement gave **the same numbers** — LCP
2.5s, CLS 0.022, render-delay ~2030ms. Those figures belong to Lighthouse's
simulated throttling on a local server, not to the effects. The like-for-like
production figure is the one in the table.
...
|---|---|---|
| 2026-08-31 | 12 | Motion pass — five effects, one commit each, **no animation library**. The decision was not taken on the brief's ~30KB estimate: `motion` had already been installed and built in this project, and `LazyMotion` + `domAnimation` + `strict` measured **38.3KB gz**, taking app code from 11.4 to 49.7KB against a 15KB budget. All five effects together cost **1.6KB**. **§A scramble** — the Japanese guard tests the *string*, not the locale, which matters specifically here: the English copy names Japanese organisations (国総研, 株式会社), so an English heading can legitimately contain CJK and a locale branch would have scrambled it. No layout shift, because the real string is rendered invisibly to hold the final width with the scramble positioned over it — Silkscreen is a pixel face, not monospace, so swapping W for I would otherwise move the width every frame. **§B count-up** — the figures are not plain numbers (`1,400`, `<1`, `40%`), so the parser splits prefix, numeric core and suffix and counts only the core; strings with no digits render untouched, which is what keeps the Japanese unit lines safe. `tabular-nums` alone was not enough: the digit *count* grows while counting, so the width is reserved the same way. **§C hairlines** — a real element rather than a border, since `transform` cannot touch a border and `width` would force layout every frame; `Reveal` gained a `rule` variant rather than a second component duplicating the trigger. **§D crosshair** — three gates (fine pointer, not reduced-motion, pointer has moved), and both media queries are *watched* rather than sampled once, since a hybrid laptop can gain or lose a fine pointer and reduced-motion can be toggled with the page open. The `mousemove` listener never writes to the DOM: it records coordinates and flags a frame, one rAF does one `translate3d`. **§E counter** — the six static per-section counters became one pinned readout driven by a single observer at `rootMargin: -50%`; `SectionShell` lost its now-redundant `total` prop and the dead `Counter` primitive was deleted. §F not built. **The one scare was not a regression**: Lighthouse against a local `next start` reported LCP 2.5s and CLS 0.022, so I checked out the pre-stage commit and ran the identical measurement — same numbers, same ~2030ms render delay. Those belong to Lighthouse's simulated throttle on localhost, not to the effects; production is 1.16s (en) and 1.33s (ja) against a 1.23s baseline. Verified per effect rather than in aggregate: en scramble shows 11 distinct states and ja exactly 1; reduced-motion gives 1 for both scramble and count-up; the crosshair is absent under both `pointer: coarse` and reduced-motion; the counter steps 01→03→04→05→06 with zero per-section counters left; rules go `scaleX(0)` → `scaleX(1)`. axe 0 violations across 30 rules. **Stage 12 deliberately left unticked** — Aayush wants to look at it with motion on and with reduced-motion forced. |
